...
#if defined(MOZJS_MAJOR_VERSION)
#if MOZJS_MAJOR_VERSION >= 33
bool js_PluginSdkboxPlayJS_PluginSdkboxPlay_saveGameDataString(JSContext *cx, uint32_t argc, JS::Value *vp)
#else
bool js_PluginSdkboxPlayJS_PluginSdkboxPlay_saveGameDataString(JSContext *cx, uint32_t argc, jsval *vp)
#endif
#elif defined(JS_VERSION)
JSBool js_PluginSdkboxPlayJS_PluginSdkboxPlay_saveGameDataString(JSContext *cx, uint32_t argc, jsval *vp)
#endif
{
    s_cx = cx;
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    ...
    if (argc == 2) {
        std::string name;
        std::string value;
        ok &= jsval_to_std_string(cx, args.get(0), &name);
        ok &= jsval_to_std_string(cx, args.get(1), &value);
        JSB_PRECONDITION2(ok, cx, false, "js_PluginSdkboxPlayJS_PluginSdkboxPlay_saveGameDataString : Error processing arguments");
        sdkbox::PluginSdkboxPlay::saveGameDataBinary(name, (void*)value.c_str(), (int)value.length());
        args.rval().setUndefined();
        return true;
    }
    JS_ReportErrorUTF8(cx, "saveGameDataString/Binary : wrong number of arguments, should be ' sdkbox.PluginSdkboxPlay.saveGameDataString('name', 'stringdata'); '");
    return false;
}
...
JS_DefineFunction(cx, pluginObj, "saveGameDataString", js_PluginSdkboxPlayJS_PluginSdkboxPlay_saveGameDataString, 1, JSPROP_READONLY | JSPROP_PERMANENT); \
JS_DefineFunction(cx, pluginObj, "saveGameDataBinary", js_PluginSdkboxPlayJS_PluginSdkboxPlay_saveGameDataString, 1, JSPROP_READONLY | JSPROP_PERMANENT); \
